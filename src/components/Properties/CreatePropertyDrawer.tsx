...
export default function CreatePropertyDrawer({
  ...
  const watchCoverImage = watch("coverImage") as File | undefined;
  const watchImages = watch("images") as File[] || [];
  const watchDeliveryDate = watch("deliveryDate");

  // Local state for image file preview URLs
  ...
  // Reset previews and form when drawer is closed
  useEffect(() => {
    if (!isOpen) {
      reset();
      setCoverPreviewUrl(null);
